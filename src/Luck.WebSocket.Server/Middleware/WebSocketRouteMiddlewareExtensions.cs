...
namespace Luck.WebSocket.Server.Extensions
{
    public static class WebSocketRouteMiddlewareExtensions
    {
        ...
        /// <returns></returns>
        public static IApplicationBuilder UseWebSocketServerUI(this IApplicationBuilder app, string path = "/luck/wsdebug")
        {
            // debug ui
            app.Map(path, (appbuilder) =>
            {
                ...
                {
                    await context.Response.WriteAsync("<h1>Luck------------->WebSocket   Test</h1>");
                });
            });
